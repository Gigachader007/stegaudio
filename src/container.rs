...
impl DataContainerXChaCha20Poly1305Lzma {
    pub fn empty(size: usize, password: String) -> Self {
        Self { map: HashMap::new(), size: size,  cipher: XChaCha20Poly1305::new(&Sha256::digest(password))}
    }
    pub fn try_new(data: Vec<u8>, password: String) -> Result<Self> {
        ...
        let cipher = XChaCha20Poly1305::new(&Sha256::digest(password));
        ...
        let raw_data = data[8+24+std::mem::size_of::<usize>()..8+24+std::mem::size_of::<usize>()+enc_size].to_vec();
        ...
        Ok(Self{map: map, size: data.len() - 24, cipher})
    }
}
